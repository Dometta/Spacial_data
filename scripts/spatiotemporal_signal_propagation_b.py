...
def get_block_lim(block): 
    ...
    min_y = block["objNuclei_Location_Center_Y"].min()

    max_x = block["objNuclei_Location_Center_X"].max()
    max_y = block["objNuclei_Location_Center_Y"].max()

    return min_x, min_y, max_x, max_y



def categorize_cell_position(block, edge_quant):
        
    #list for all upcoming dfs (one df = info about one time point)
    parts = []

    #since the cells might change their positions, assigning categories for
    #all visible cells (track id for a specific time point) at one time point at a time (Image metadata t)

    for image, image_df in block.groupby("Image_Metadata_T"):

        #working on a copy
        image_df = image_df.copy()

        #before computing the lim, getting rid of Nans
        ...
        #getting the "range" of the frame in the block
        min_x, min_y, max_x, max_y = get_block_lim(image_df)


        x = image_df["objNuclei_Location_Center_X"]
        y = image_df["objNuclei_Location_Center_Y"]

        #computing distances to the nuclei on the perimeters
        to_left = x - min_x
        to_right = max_x - x
        ...
        #for a Series obj
        #computing the smallest distance from those nuclei
        min_distance = pd.Series(np.minimum.reduce([to_left,
                                                    to_right, to_top, to_bottom
                                                    ]), index = image_df.index)
        
    
        thresh = min_distance.quantile(edge_quant)

        #adding new values
        image_df["edge_distance"] = min_distance

        #assigning the categories (based on the threshold)
        image_df['spatial_category'] = np.where(
            image_df['edge_distance'] <= thresh,
            'edge',
            'central',
        )

        parts.append(image_df)
...
def save_outputs(block: pd.DataFrame, spatial_edges: pd.DataFrame, temporal_edges: pd.DataFrame, summary: dict, output_dir: Path) -> None:
    ...
    #added new colnames for classification===========================
    node_cols = [
        ...
        'neighbor_count', 'neighbor_jump_count', 'neighbor_jump_now',
         'neighbor_mean_signal', 'future_self_jump','edge_distance', 'spatial_category'
    ]
